...
// $(A).prepend(B) : A 요소의 자식 요소로 B 요소를 앞부분에 추가
...
$("<span>apeendTo</span>").appendTo("#area2");
...
$(".item").hover(
  ...
  (event) => {
    console.log("evnet3 : " + event.type);
    $(event.target).toggleClass("bg-hotpink");
  }
);

$("#btn1").click(() => {
  // 이벤트까지 복제하려면 매개값을 true를 전달 (기본값 : false)
